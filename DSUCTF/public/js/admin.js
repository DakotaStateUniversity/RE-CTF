...
$(document).ready(function() {
		...
		$('#competition_start').datetimepicker({theme:'dark'});
		$('#competition_end').datetimepicker({theme:'dark'});
...
{
	$.ajax({
		url: '/ajax/competition/start_time',
		type: 'GET',
		success: function(response) {
			if(response == "-0001-11-30 00:00:00")
				$("#competition_start").val("");
			else
				$("#competition_start").val(response);
		}
	});
	$.ajax({
		url: '/ajax/competition/end_time',
		type: 'GET',
		success: function(response) {
			if(response == "-0001-11-30 00:00:00")
				$("#competition_end").val("");
			else
				$("#competition_end").val(response);
		}
	});
}
...
function updateTime()
{
	var comp_start = $("#competition_start").val() == "" ? "0000-00-00 00:00:00" : $("#competition_start").val();
	var comp_end = $("#competition_end").val() == "" ? "0000-00-00 00:00:00" : $("#competition_end").val();

	$.ajax({
		url: '/ajax/competition/update_time',
		type: 'GET',
		data: {competition_start: comp_start, competition_end: comp_end},
		success: function(response) {
